...
test.describe('', () => {
  ...
  test('【取消】【购销合同】【预收款】订单节点: SAP返回ECCID && 取消SAP返回成功 && 重新生成购销合同 && 合同状态: 待用印审批', async ({
    page,
    copyOrderPage,
    approvalPage,
    contractPage,
    ...
    await customTestStep.b2bTestStep('4. 取消商务审核通过', async () => {
      await cancelPage.businessApproval(orderId!, '审核通过')
    })
    await customTestStep.b2bTestStep('5. 取消运营审核通过', async () => {
      await cancelPage.manualApproval(orderId!, '审核通过')
      await orderApi.createCancelSapReturn(orderId!, 'S')
    })
    await customTestStep.b2bTestStep('6. 查看购销合同', async () => {
