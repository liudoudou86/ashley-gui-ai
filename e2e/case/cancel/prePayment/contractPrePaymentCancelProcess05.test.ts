...
test.describe('', () => {
  ...
  test('【取消】【购销合同】【预收款】订单节点: 商务审核-订单下发OMS && 取消运营审核否决 && 重新生成购销合同 && 合同状态: 待用印审批', async ({
    page,
    copyOrderPage,
    approvalPage,
    contractPage,
    ...
    await customTestStep.b2bTestStep('5. 取消运营审核否决', async () => {
      await cancelPage.manualApproval(orderId!, '审核否决')
    })
