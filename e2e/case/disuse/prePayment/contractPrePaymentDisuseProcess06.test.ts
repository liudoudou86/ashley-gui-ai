...
test.describe('', () => {
  ...
  test('【购销合同】【预收款】合同节点: 已盖章 && 盖章状态: 甲方乙方已盖章 && 核查通过 && 总部特殊作废 && 不生成购销合同 && 合同状态: 作废中', async ({
    page,
    copyOrderPage,
    approvalPage,
    contractPage,
    ...
    await customTestStep.b2bTestStep('6. 核查通过', async () => {
      await checkPage.check(orderId!, '通过')
    })
    await customTestStep.b2bTestStep('7. 总部特殊作废', async () => {
      await disusePage.headquartersSpecialDisuse(orderId!)
    })
    await customTestStep.b2bTestStep('8. 查看购销合同', async () => {
